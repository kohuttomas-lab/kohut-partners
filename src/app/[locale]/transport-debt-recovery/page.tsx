...
import styles from "./transport-debt-recovery.module.css";

/* Landing page k vymáhaniu prepravných pohľadávok podľa Dohovoru CMR.
   Cieľová skupina: dopravcovia a špedície (SK verzia slovenskí, EN verzia
   zahraniční, najmä poľskí a litovskí), ktorým slovenský objednávateľ
   neuhradil prepravné.

   Copy musí obstáť voči § 29b zák. 586/2003 o advokácii — len vecné
   objektívne informácie, žiadne porovnávanie s inými advokátmi, žiadne
   sľuby výsledku. Odmena naviazaná na výsledok sa opisuje podľa § 7 ods. 2
   a § 8 vyhl. 655/2004. Všetky texty sú v messages/{sk,en}.json,
   namespace "cmr". */
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "cmr" });
  const title = t("metaTitle");
  const description = t("metaDescription");
  return {
    title,
    description,
    alternates: localeAlternates(locale, "/transport-debt-recovery"),
    openGraph: {
      title,
      description,
      type: "website",
      url: absoluteUrl(locale, "/transport-debt-recovery"),
      images: [{ url: ogImageUrl(locale) }],
    },
  };
}

export default async function TransportDebtRecoveryPage(props: Props) {
  const { locale } = await props.params;
  ...
  // Súvisiaca oblasť práva — názov berieme z katalógu služieb, aby sa nemusel
  // prekladať druhýkrát.
  const relatedService = getService(locale as Locale, "spory");
        ...
        <Container style={{ maxWidth: 880 }}>
          ...
          <div className={styles.related}>
            ...
            <div className={styles.relatedLinks}>
              {relatedService ? (
                <Link href={{ pathname: "/services/[id]", params: { id: "spory" } }}>
                  {relatedService.name}
                </Link>
              ) : null}
